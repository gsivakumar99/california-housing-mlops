...
from src.train import train_and_log_model, train_models
...
def test_train_and_log_model(mock_data):
    ...
    with patch("mlflow.start_run") as mock_start_run, \
        ...
        train_and_log_model(model, "Linear Regression", X_train, X_test,
                            y_train, y_test)
        ...
        assert hasattr(model, "coef_"), "Model coefficients not found."
        "Training failed."
...
def test_train_models(mock_data):
    ...
    with patch("src.train.load_data") as mock_load_data, \
         patch("src.train.preprocess_data") as mock_preprocess_data, \
         patch("src.train.train_and_log_model") as mock_train_and_log_model, \
...
def test_individual_models(mock_data, model_name, model):
    ...
    with patch("mlflow.start_run") as mock_start_run, \
        ...
        train_and_log_model(model, model_name, X_train, X_test,
                            y_train, y_test)
        ...
        assert hasattr(model, "predict"), "Model training"
